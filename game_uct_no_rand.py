...
from playout_compare import simulate_game_given_rng, RNGOrder
import random
import math
import logging

MAX_DEPTH = 35
NUM_SIMULATIONS_PER_CHOICE = 5
...
def MCTS_no_rand(root_node: Node, max_depth: int, num_simulations: int, this_rng: RNGOrder) -> int:
    """
    Perform Monte Carlo Tree Search (MCTS) on the game tree.
    This function will simulate random games from the current state
    and update the tree with the results. An RNGOrder object
    is used to ensure consistent random outcomes.
    """
    for sim in range(num_simulations):
        ...
        # Simulation
        result, _, _ = simulate_game_given_rng(
...
def run_mcts_no_rand(root_node:Node, num_simulations: int, this_rng: RNGOrder) -> int:
    """
    Run MCTS on the given game state.
    This function will create a root node and perform MCTS on it.
    """
    best_action = MCTS_no_rand(root_node, MAX_DEPTH, num_simulations, this_rng)
    return best_action
...
if __name__ == "__main__":
    logging.basicConfig(
        filename=f'uctnr-{UCT_CONSTANT}-{MIN_BUDGET}_seed-{SEED}.log',
        # level=logging.DEBUG,
        # level=logging.INFO,
    ...
    while gs.phase != logic.PHASE_END_GAME:
        ...
        if gs.current_choice is not None:
            ...
            best_move = run_mcts_no_rand(current_node, get_num_simulations(gs), rng.get_copy())
            full_log("-" * 20, logger)
