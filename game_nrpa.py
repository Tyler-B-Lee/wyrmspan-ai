...
NUM_ITERATIONS = 50  # Number of iterations for NRPA
ALPHA = 0.1  # Alpha value for NRPA, used in policy adaptation
NRPA_LEVEL = 2  # Level of NRPA to run, can be adjusted for deeper searches

SEED = 6302025
AUTOMA_DIFFICULTY = 0 # Difficulty level for the automa, can be adjusted
...
def NRPA(level: int, policy: defaultdict, root: GameState) -> tuple:
    """
    ...
    if level == 0:
        ...
        if gs.player.score >= gs.automa.score:
            score += 0.75
        # return score * outcome_prob, sequence, policy
        ...
        for i in range(NUM_ITERATIONS):
            if level == NRPA_LEVEL and i % 10 == 0:
                logger.warning(f">>> Iteration {i+1}/{NUM_ITERATIONS} at level {level}")
            result, new, _new_policy = NRPA(level - 1, policy, root)
            if result >= best_score:
                if level == NRPA_LEVEL:
                    logger.warning(f"\tNew best score found: {result}")
                # logger.warning(f"\tNew sequence: {new}")
                best_score = result
                sequence = new
...
if __name__ == "__main__":
    ...
    while game.phase != logic.PHASE_END_GAME:
        ...
        if game.current_choice is not None:
            ...
            best_score, sequence, policy = NRPA(NRPA_LEVEL, policy, game)
